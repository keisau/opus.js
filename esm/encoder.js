...
export class Encoder {
  ...
  async init() {
    ...
    try {
      const { default: create } = await import('../build/opus-encoder')

      opus = this.opus = create()
    } catch (err) {
      const { default: create } = await import('../build/opus-encoder.asm')

      opus = this.opus = create()
    }

    this.component = new opus.Encoder(this.sampleRate, this.channels, this.application)
